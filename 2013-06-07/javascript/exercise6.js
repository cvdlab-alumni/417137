...
function road1(numStrade){
	arr = [];
	for (var i = 0; i < numStrade; i++) {
		arr.push(T([0])([i])(CUBOID([0.2,6,0.01])));
		arr.push(T([1])([i])(CUBOID([6,0.2,0.01])))
	};
return arr;
}
...
DRAW(T([0,1])([6.8,15.8])(STRUCT(road1(7))));
DRAW(T([0,1])([-10.2,19.8])(STRUCT(road1(7))));

//autostrada

autostrada=T([0,1])([-4,19.8])(CUBOID([13,0.5,0.01]))

autostrada2=T([1])([1.5])(autostrada)
DRAW(autostrada)
DRAW(autostrada2)
...
FV=model[1];
var stringOutput="";
stringOutput +=  "# List of Vertices, with (x,y,z[,w]) coordinates, w is optional and defaults to 1.0." + "\n"

for (var i = 0; i < V.length; i++) {
		stringOutput += "v "+ V[i][0] + " " + V[i][1]
		if(typeof(V[i][2])!==undefined)
			stringOutput += " " + "0"  + " " + "1.0" + "\n"
		else
			stringOutput += " " + V[i][2] + " " + "1.0" + "\n"
	};

stringOutput+="\n";

stringOutput += "# Face Definitions" + "\n"
	for (var i = 0; i < FV.length; i++) {
				stringOutput += "f "+ FV[i][0] + " " + FV[i][1] + " " + FV[i][2] + "\n"
